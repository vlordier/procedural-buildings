...
class Scope:
    ...
    def scale(self, factor):
        factors = np.array([factor, factor, factor]) if np.isscalar(factor) else np.array(factor)
        newSize = self.size * factors
        return Scope(self.pos, self.rotMat, newSize)
    ...
    def inset(self, amount):
        ...
        right = Scope(
            self.pos + self.rotMat.dot(np.array([s[0] - d, 0, d])), self.rotMat, np.array([d, s[1], s[2] - 2 * d])
        )
        inner = Scope(
            self.pos + self.rotMat.dot(np.array([d, d, d])), self.rotMat, np.array([s[0] - 2 * d, s[1], s[2] - 2 * d])
        )
    ...
    def toMat(self):
        ...
        scaleReCentreTranslate = np.array([
